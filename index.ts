 ...
 * @returns boolean: True if the file is available on the network, false otherwise.
 */
exports.fileProofDownload = async (
    ipfsNode: any,
    CID: String,
    options: {timeout?: number} = {}
) => {
...
exports.baoStream = async (
    ...
    const source = await toBuffer(ipfsNode.cat(CID, {offset: start, length: size, timeout: proofTimeout}));
    return source;
}
 ...
 */
exports.getChallengeBlockCID = async (
    ipfsNode: any, 
    CID: String
) => {
    // Get all the block IDs for the file
    const stat = await ipfsNode.object.stat(CID)
    console.log("Num Links: ", stat.NumLinks)
    if (stat.NumLinks == 0 )
    {
        const source = ipfsNode.cat(CID)   
        const hash = (await ipfsNode.add(source, {onlyHash: true}, {pin: false})).cid.toString()
        if (hash == CID)
        {
            return true
        }
    ...
    else
    {
        ...
        if (hashes[index])
        {
            ...
            source = await ipfsNode.cat(block_cid)
        }
        else
        {
            throw new Error("No block of that index")
        }
        const hash_cat = (await ipfsNode.add(source, {onlyHash:true})).cid.toString()
